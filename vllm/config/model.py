...
class ModelConfig:
    ...
    def _verify_quantization(self) -> None:
        ...
        if quant_cfg is not None:
            resolution = me_quant.resolve_quantization_method(
                quant_cfg, self.quantization, hf_config=self.hf_config
            )
            self.quantization = cast(
                me_quant.QuantizationMethods | None,
                resolution.selected_method,
            )

        if self.quantization is not None:
            me_quant.validate_quantization_method(self.quantization)

        if me_quant.is_deprecated_quantization_method(self.quantization):
